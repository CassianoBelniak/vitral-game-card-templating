...
export class ComponentText extends Component {
    ...
    async getValues(variables: { [key: string]: string } = {}) {
        ...
        const values = {
            ...
            lineHeight: new Parser(this.lineHeight).default(String(fontSize)).variables(variables).toPixels(),
            bottomMargin: new Parser(this.bottomMargin).variables(variables).default('0px').toPixels(),
            topMargin: new Parser(this.topMargin).variables(variables).default('0px').toPixels(),
            leftMargin: new Parser(this.leftMargin).variables(variables).default('0px').toPixels(),
            rightMargin: new Parser(this.rightMargin).variables(variables).default('0px').toPixels(),
            guideWidth: width,
            guideHeight: height,
        }
