    ...
    function filterFiles(val: string, update: (a: () => void) => void) {
        update(() => {
            if (val === '') {
                ...
                const needle = val.toLowerCase()
                availableFiles.value = getAllCardFiles().filter(
                    v => v.toLowerCase().indexOf(needle) > -1
                )
            }
        })
...
<template>
    <q-scroll-area class="h-full">
        <q-card class="p-2 my-2">
            ...
            <q-select use-input @filter="filterTags" @new-value="createTagValue" class="mb-2 col-auto tags" dense
                outlined label="Tags" v-model="model.tags" multiple :options="availableTags" use-chips stack-label />
            <q-select use-input @filter="filterFiles" @new-value="createFileValue" class="mb-2 col-auto tags" dense
                outlined label="Source file" v-model="model.source" :options="availableFiles" use-chips stack-label />
