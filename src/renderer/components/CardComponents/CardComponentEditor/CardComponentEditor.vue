...
<template>
    <div class="row justify-between cursor-pointer" @click="isMainSectionOpen = !isMainSectionOpen">
        ...
        <div class="my-2 mr-2 template-counter">
            +{{ model.length }}
            <q-tooltip> +{{ model.length }} templates added to this section </q-tooltip>
        </div>
    </div>
    <q-slide-transition>
        <div class="mt-2 reverse column" v-show="isMainSectionOpen">
            <template v-for="(templateName, index) in model" :key="templateName">
                <div class="row line">
                    ...
                    <div class="handlers">
                        <TemplateHandlers @delete="onDelete(index)" @move-down="onMoveDown(index)" @move-up="onMoveUp(index)" @duplicate="onDuplicate(index)" />
                    </div>
                </div>
            </template>
        </div>
    </q-slide-transition>
    <TemplateSelector class="add-button" @template-selected="onAddTemplate" :card-id="props.cardId" />
</template>
