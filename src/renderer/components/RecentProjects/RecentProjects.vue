...
<template>
    ...
    <div vertical align="left" v-for="item in recentProjects" :key="item.path">
        <q-btn class="load-button" push align="left" no-caps @click="onClickProject(item.path)">{{ item.label }}</q-btn>
        <q-btn push align="left" no-caps @click="removeProject(item.path)" icon="remove">
            <q-tooltip> Remove project from recents </q-tooltip>
        </q-btn>
    </div>
    <alert-dialog v-model="alertOpen" title="Can't open project" content="It seems the projects was moved or don't exists anymore" />
</template>
