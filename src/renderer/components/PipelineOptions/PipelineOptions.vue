...
<template>
    <q-scroll-area class="w-full h-full">
        <name-field class="mb-2" v-model="model.name" :current-name="props.pipelineName" :existing-names="Object.keys(exportPipelinesStore.exportPipelines)" />
        ...
        <q-btn class="my-2 full" no-caps push @click="showSelectCardModal = true"
            >Select cards
            <div class="ml-2 text-gray-400"></div>
        </q-btn>
        <q-select class="mb-2" v-model="model.exportType" label="Export type" dense outlined :options="exportTypes" emit-value map-options></q-select>
        <PipelineSizeFields
            label="Paper size"
            v-model:x="model.paperWidth"
            v-model:y="model.paperHeight"
                            ...
                            <q-menu :model-value="true" anchor="top right" self="top left">
                                <q-list style="min-width: 100px">
                                    <q-item
                                        clickable
                                        v-close-popup
                                        v-for="option in commonPaperSizes"
                                        @click="onCommonPaperSizeSelected(option.sizes)"
                                        v-bind:key="option.sizes"
                                    >
        ...
        <PipelineSizeFields label="Margin" v-model:x="model.marginX" v-model:y="model.marginY" :is-visible="!!optionVisibility[model.exportType]?.margin" />
        <PipelineSizeFields
            label="Bleeding"
            v-model:x="model.bleedingX"
            v-model:y="model.bleedingY"
            :is-visible="!!optionVisibility[model.exportType]?.bleeding"
        />
        <PipelineSizeFields
            label="Backside offset"
            v-model:x="model.backsideOffsetX"
            v-model:y="model.backsideOffsetY"
            :is-visible="!!optionVisibility[model.exportType]?.backsideOffset"
        />
        <PipelineSizeFields
            label="Frontside offset"
            v-model:x="model.frontsideOffsetX"
            v-model:y="model.frontsideOffsetY"
            :is-visible="!!optionVisibility[model.exportType]?.frontsideOffset"
        />
        <PipelineSizeFields
            label="Card sides spacing"
            v-model:x="model.cardSidesSpacing"
            v-model:y="model.cardSidesSpacing"
            :is-visible="!!optionVisibility[model.exportType]?.cardSideSpacing"
            :hide-y="true"
        />
